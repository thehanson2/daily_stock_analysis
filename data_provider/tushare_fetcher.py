    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        try:
            ...
            if df_perf is not None and not df_perf.empty:
                # 检查必要列
                if 'avg_cost' not in df_perf.columns or 'profit_ratio' not in df_perf.columns:
                    logger.warning(f"[Tushare] cyq_perf 缺少必要列，实际列: {df_perf.columns.tolist()}")
                    raise ValueError("cyq_perf 缺少 avg_cost 或 profit_ratio")
