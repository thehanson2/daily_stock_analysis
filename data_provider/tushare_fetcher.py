...
class TushareFetcher(BaseFetcher):
    ...
    def _fetch_raw_data(self, stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            if is_hk:
                ...
                raise DataFetchError("Simulated failure for ETF daily data")
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        数据来源：
        - 平均成本、获利比例：ts.pro_api().cyq_perf() （更准确，避免计算偏差）
        - 90/70 成本区间及集中度：ts.pro_api().cyq_chips() + compute_cyq_metrics
        ...
        try:
            ...
            if df_chips is None or df_chips.empty:
                logger.warning(f"[Tushare] 未获取到 {stock_code} 的 cyq_chips 数据")
                return None
            ...
            try:
                ...
                logger.warning(f"[Tushare] cyq_perf 接口调用失败，将使用 cyq_chips 自行计算: {e}")
            ...
            if daily_df is None or daily_df.empty:
                logger.warning(f"[Tushare] 未获取到 {stock_code} 的日线数据，无法计算集中度")
                return None
            ...
            # 6. 从 cyq_perf 获取 avg_cost 和 profit_ratio（若成功），否则 fallback 到 metrics 中的值
            if df_perf is not None and not df_perf.empty:
                avg_cost = float(df_perf.iloc[0]['avg_cost'])
                profit_ratio = float(df_perf.iloc[0]['profit_ratio']) / 100.0  # cyq_perf 返回的是百分比（如 35.2 表示 35.2%），转为小数
                logger.debug(f"[筹码分布] 使用 cyq_perf: avg_cost={avg_cost}, profit_ratio={profit_ratio:.4f}")
            else:
                avg_cost = metrics['平均成本']
                profit_ratio = metrics['获利比例']
                logger.debug(f"[筹码分布] 降级使用 cyq_chips 计算: avg_cost={avg_cost}, profit_ratio={profit_ratio:.4f}")
