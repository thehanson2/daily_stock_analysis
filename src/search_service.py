...
class SearchService:
    ...
    def search_comprehensive_intel(
        ...
        if not is_foreign and not is_index_etf:
            try:
                cninfo_resp = self._direct_cninfo_announcements(stock_code, stock_name, limit=3)
                if cninfo_resp.success and cninfo_resp.results:
                    results['cninfo_announcements'] = cninfo_resp
                    logger.info(f"[情报搜索] 巨潮直连公告: {len(cninfo_resp.results)} 条")
            except Exception as e:
                logger.warning(f"[情报搜索] 巨潮直连公告失败: {e}")
        # =======================================================================
    ...
    def _recent_day_range(self, lookback_days: int = 180):
        end = datetime.now()
    ...
    def format_intel_report(self, intel_results: Dict[str, SearchResponse], stock_name: str) -> str:
        ...
        # 维度展示顺序
        display_order = ['latest_news', 'announcements', 'market_analysis', 'risk_check', 'earnings', 'industry', 'cninfo_announcements']
